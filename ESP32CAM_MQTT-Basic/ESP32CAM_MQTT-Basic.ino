...
//Datos de WiFi
const char* ssid = "INFINITUM359B";  // Aquí debes poner el nombre de tu red
const char* password = "2uYYpD3bHK";  // Aquí debes poner la contraseña de tu red

//Datos del broker MQTT
const char* mqtt_server = "192.168.1.96"; // Si estas en una red local, coloca la IP asignada, en caso contrario, coloca la IP publica
...
void loop() {
  ...
  if (timeNow - timeLast > wait) { // Manda un mensaje por MQTT cada cinco segundos
    ...
    client.publish("codigoIoT/ejemp/MQTT", dataString); // Esta es la función que envía los datos por MQTT, especifica el tema y el valor
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == "codigoIoT/ejemp/MQTT") {  // En caso de recibirse mensaje en el tema esp32/output
    if(messageTemp == "true"){
  ...
  }// fin del if (String(topic) == "codigoIoT/ejemp/MQTT")
...
void reconnect() {
  ...
  while (!client.connected()) { // Pregunta si hay conexión
    ...
    if (client.connect("ESP32CAMClient")) { //Pregunta por el resultado del intento de conexión
      ...
      client.subscribe("codigoIoT/ejemp/MQTT"); // Esta función realiza la suscripción al tema
